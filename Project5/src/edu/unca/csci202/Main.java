...
/**
 * 
 * driver class to time the linear complexity solution vs the logarithmic
 * complexity solution
 * 
 * @author ksanft
 *
...
public class Main {
	...
	for (int i = 0; i < 8; i++) {
	    Model m = createModel(copies);
	    HashMinHeapLogarithmic<NRMReaction> heap2 = new HashMinHeapLogarithmic<NRMReaction>(
		    m.getNumberOfReactions());
	...
	for (int i = 0; i < copies; i++) {
	    ...
	    r[i * 4] = new Reaction("infection(from asymptomatic)" + i, 0.00001,
		    ...
		    new Species[] { s[i * 4 + 1] }, new double[] { 2 });
	    r[i * 4 + 1] = new Reaction("infection(from symptomatic)" + i, 0.000001,
		    ...
		    new Species[] { s[i * 4 + 1], s[i * 4 + 2] }, new double[] { 1, 1 });
	    r[i * 4 + 2] = new Reaction("become symptomatic" + i, 1.0 / 5,
		    // exposed becomes infected
		    new Species[] { s[i * 4 + 1] }, new double[] { 1 }, new Species[] { s[i * 4 + 2] },
		    new double[] { 1 });
	    r[i * 4 + 3] = new Reaction("recover" + i, 1.0 / 14, new Species[] { s[i * 4 + 2] }, new double[] { 1 },
		    new Species[] { s[i * 4 + 3] }, new double[] { 1 });
	}
